...
Proportional to blast radius: trivial change (typo/rename/<20 lines, one obvious form) → apply mentally and just do it; non-trivial → stages visible in the response. Elaborated guidance + worked example: `Skill(dev)`.
...
5. **LEARN** — persist the non-obvious (memory/learning capture): what surprised, what pattern emerged, what was deferred and its upgrade trigger.
