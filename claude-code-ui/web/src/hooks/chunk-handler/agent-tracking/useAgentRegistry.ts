...
export function useAgentRegistry({ setLogs }: UseAgentRegistryOptions) {
  ...
  const registerAgent = useCallback(
    (toolUseId: string, subagentType: string, description: string): ActiveAgent => {
      ...
      const displayName =
        agentIndex > 1 ? `Agent: ${subagentType} #${agentIndex}` : `Agent: ${subagentType}`
  ...
  const addStepToAgent = useCallback(
    (parentId: string, step: AgentStep): boolean => {
  ...
  const completeStepInAgent = useCallback(
    (parentId: string, toolName: string): void => {
      const agent = activeAgentsMapRef.current.get(parentId)
      if (!agent) return
