    ...
    (chunk: StreamChunk) => {
      ...
      switch (chunk.type) {
        ...
        case 'tool_use': {
          ...
          const now = new Date()
          const activeAgent =
            toolType === 'agent' && chunk.toolUseId
        ...
        case 'tool_result': {
          ...
          const agentByToolUseId = chunk.toolUseId
            ? agentTracking.activeAgentsMapRef.current.get(chunk.toolUseId)
            : null
          const parentAgentIdFromMap = chunk.toolUseId
            ? agentTracking.toolToParentMapRef.current.get(chunk.toolUseId)
            : null
          ...
          if (
            ...
            parentAgentIdFromMap &&
            parentAgentIdFromMap !== '__parallel__'
          ) {
          ...
          if (chunk.tool && chunk.toolUseId) {
            setLogs((prev) => {
              const idx = prev.findIndex(
                (log) => log.type === 'tool' && log.toolUseId === chunk.toolUseId && !log.toolOutput
              )
          ...
          if (chunk.usage && setUsage) {
            ...
            setUsage((prev) => {
              ...
              return {
                ...
                cacheCreationTokens:
                  (prev.cacheCreationTokens || 0) + (chunk.usage!.cacheCreationTokens || 0),
        ...
        case 'context': {
          ...
          setActiveContext((prev) => ({
            ...
            ...(contextChunk.contextType === 'rule'
              ? {
                  rules: prev.rules.includes(contextChunk.name)
                    ? prev.rules
                    : [...prev.rules, contextChunk.name],
                }
              : {}),
          }))
          break
        ...
        case 'agent_event': {
          ...
          agentTracking.handleAgentEvent(
            event,
            agentId,
            agentType,
            task,
        ...
        case 'learning_event': {
          if (onLearningEvent) {
            ...
            onLearningEvent({
              type: `learning:${event}` as
                | 'learning:started'
                | 'learning:completed'
                | 'learning:failed',
              expertId,
              changes: changes as LearningChange[],
              error: learningError,
            })
          }
          break
