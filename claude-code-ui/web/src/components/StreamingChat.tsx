...
export default function StreamingChat({
  ...
  const [claudeConfig, setClaudeConfig] = useLocalStorage<ClaudeConfig>(
    STORAGE_KEYS.CLAUDE_CONFIG,
    { agents: [], skills: [], commands: [] }
  )
  const [isContextPanelOpen, setIsContextPanelOpen] = useLocalStorage(
    STORAGE_KEYS.CONTEXT_PANEL,
    true
  )
  ...
  useEffect(() => {
    getSessionAgents(session.id).then((result) =>
      actionsRef.current.setSessionAgents(result.agents)
    )
  }, [session.id])

  useEffect(() => {
    const completedAgents = state.activeContext.toolHistory.filter(
      (t) => t.type === 'agent' && t.status === 'completed' && t.toolUseId
    )
  ...
  useEffect(() => {
    ...
    // session.messages is an unstable array ref; use .length to avoid spurious reruns
  ...
  const sendMessage = useCallback(
    (prompt: string, images?: { dataUrl: string; name: string }[]) => {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault()
      if ((!state.input.trim() && pastedImages.length === 0) || state.isProcessing || !isConnected)
        return
    ...
    <div className="h-full flex flex-col bg-surface-primary">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col relative">
          <FilterBar filter={filter} onFilterChange={setFilter} logs={state.logs} />
