...
// -- Outloop Mode Types - Phase 4 --

export type OutloopStatus =
  | "queued"
  | "running"
  | "retrying"
  | "completed"
...
export interface OutloopRun {
  id: string;
  prompt: string;
  blueprintId?: string;
  status: OutloopStatus;
  currentRound: number;
  maxRounds: number;
  startedAt: string;
  completedAt?: string;
...
export interface OutloopStartRequest {
  prompt: string;
  blueprintId?: string;
  workDir?: string;
  maxRounds?: number;
