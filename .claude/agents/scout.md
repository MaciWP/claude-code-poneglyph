...
- **Gather Context**: Provide pre-implementation intelligence for builder agent (planning lives in the `tech-plan` skill)
...
| Lead invokes `tech-plan` skill | Architecture map, dependency graph |
...
> Surface this section in your final report so it can be carried into your memory file (`.claude/agent-memory/scout/MEMORY.md`). There is no automatic SubagentStop hook.
