...
description: |
  Testing strategies with Bun Test — unit, integration, mocking, fixtures and TDD workflow.
  
  Use when: writing or reviewing tests, setting up test infrastructure, coverage improvement.
  Keywords - test, mock, tdd, coverage, unit, integration, fixture, bun test, e2e, bdd
disable-model-invocation: false
argument-hint: "[test file, pattern, or module]"
effort: medium
---
...
Mutation score > 80% is a stronger quality gate than code coverage > 80%. Coverage measures what code runs; mutation score measures whether tests actually catch changes.
...
When coverage is high but bugs still appear, check mutation score.

## Edge-Case Auto-Detection

When generating tests, auto-detect edge cases from keywords in the code:

| Code keyword | Generate test for |
|-------------|-------------------|
| `valid`, `isValid` | Invalid input, empty input, boundary values |
| Number parameters | 0, -1, MAX_SAFE_INTEGER, NaN, Infinity |
| String parameters | Empty `""`, very long, special characters, unicode |
| Array/list | Empty `[]`, single item, large collection |
| Date/time | Past, future, midnight, DST boundaries |
| Optional/nullable | `null`, `undefined`, missing key |

## Always-Generate Error Cases

Every test suite should include at least one test for:
- Network failure (timeout, connection refused)
- Database error (constraint violation, connection lost)
- Permission denied (unauthorized, forbidden)
- Validation failure (malformed input, missing required fields)
