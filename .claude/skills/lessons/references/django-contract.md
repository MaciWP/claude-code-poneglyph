# OpenAPI contract lessons (binora-backend)

Migrated 2026-08-06 from `django-review-lessons/references/contract-lessons.md`. IDs C1-C19.
...
**Detection**: `grep -rn "type: object" binora-contract/openapi.yaml` — look for inline schemas that should be refs.
...
**Detection**: `grep -rn "type: array" binora-contract/openapi.yaml` — verify list endpoints have pagination wrapper.
...
**Detection**: `grep -rn '"string"\|"example"' binora-contract/openapi.yaml` — generic placeholder values.
...
**Detection**: `grep -rn "enum:" binora-contract/openapi.yaml` — look for duplicate enum definitions.
...
**Detection**: `git log --diff-filter=M -p binora-contract/openapi.yaml | grep "enum:"` — check for removed enum values.
