...
3. **`model` is an optional frontmatter field** — omit it for most agents (the Lead selects dynamically). Add `model: opus` only for strategic/high-effort agents that invariably need it (e.g. planner, builder, reviewer, error-analyzer).
