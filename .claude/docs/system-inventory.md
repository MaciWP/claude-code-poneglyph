...
| Codex / OpenAI | `~/.codex/AGENTS.md` **generated** = CLAUDE.md + poneglyph-sp.md (Codex has no append include; `model_instructions_file` REPLACES built-ins — never use it). Budget: 32 KiB (`project_doc_max_bytes`) | **re-sync after editing either source** | `bun .claude/scripts/sync-codex.ts --execute --backup --force` |
| Grok Build | `~/.grok/rules/poneglyph-sp.md` symlink only (do **not** also link the style — that double-loads the same law) | live | `ln -sfn <repo>/.claude/system-prompts/poneglyph-sp.md ~/.grok/rules/poneglyph-sp.md` (Windows: copy the file instead) |
...
claude() { ~/.local/bin/claude --append-system-prompt-file \
  "$HOME/Desktop/Bjumper/PERSONAL/REPO/claude-code-poneglyph/.claude/system-prompts/poneglyph-sp.md" "$@"; }
```
...
| Output-styles | 1 (caveman) | 1 | **1 (poneglyph)** | SSOT: `output-styles/poneglyph.md`. Claude loads via `outputStyle: Poneglyph`. Grok has no output-style feature → always-on `~/.grok/rules/poneglyph-sp.md` **symlink to the generated twin** (do not also link the style). |
