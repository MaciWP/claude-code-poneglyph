...
| Hooks | 15+ | 6 | **the ones `settings.json.hooks` registers** (5 snapshot 2026-08-07 — `auto-approve` + `session-start-plans` cut post-audit; `workspace-hint` relocated in; `code-validator` + `learning-inbox` cut in 030) | authoritative list = settings.json; event table with per-hook detail: `rules/paths/hooks.md` |
