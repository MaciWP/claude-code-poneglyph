...
describe("getLanguageFamily", () => {
  test("returns typescript for .ts and .tsx", () => {
    expect(getLanguageFamily("file.ts")).toBe("typescript");
    expect(getLanguageFamily("file.tsx")).toBe("typescript");
  });

  test("returns javascript for .js and .jsx", () => {
    expect(getLanguageFamily("file.js")).toBe("javascript");
    expect(getLanguageFamily("file.jsx")).toBe("javascript");
  });

  test("returns python for .py", () => {
    expect(getLanguageFamily("script.py")).toBe("python");
  });

  test("returns go for .go", () => {
    expect(getLanguageFamily("main.go")).toBe("go");
  });

  test("returns rust for .rs", () => {
    expect(getLanguageFamily("lib.rs")).toBe("rust");
  });

  test("returns c for .c, .cpp, .h", () => {
    expect(getLanguageFamily("main.c")).toBe("c");
    expect(getLanguageFamily("main.cpp")).toBe("c");
    expect(getLanguageFamily("header.h")).toBe("c");
  });

  test("returns unknown for unrecognized extensions", () => {
    expect(getLanguageFamily("file.lua")).toBe("unknown");
    expect(getLanguageFamily("Makefile")).toBe("unknown");
    expect(getLanguageFamily("file.md")).toBe("unknown");
  });
});

describe("isBiomeSupported", () => {
  test("returns true for supported extensions", () => {
    for (const ext of ["ts", "tsx", "js", "jsx", "json", "css", "graphql"]) {
      expect(isBiomeSupported(`file.${ext}`)).toBe(true);
    }
  });

  test("returns false for unsupported extensions", () => {
    expect(isBiomeSupported("file.py")).toBe(false);
    expect(isBiomeSupported("file.go")).toBe(false);
    expect(isBiomeSupported("file.rs")).toBe(false);
    expect(isBiomeSupported("file.md")).toBe(false);
  });
});
...
describe("isPythonFile", () => {
  ...
  test("returns false for non-Python files", () => {
    expect(isPythonFile("file.ts")).toBe(false);
    expect(isPythonFile("file.pyx")).toBe(false);
  });
});
