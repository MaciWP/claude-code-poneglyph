...
let contextPct =
  estimatedTokens > 0 ? Math.floor((estimatedTokens * 100) / CONTEXT_LIMIT) : 0;
...
// Mode indicators
const modes: string[] = [];
if (process.env.CLAUDE_CAREFUL_MODE === "true")
  modes.push(`${yellowBold}[CAREFUL]${reset}`);
if (process.env.CLAUDE_FREEZE_MODE === "true")
  modes.push(`${redBold}[FREEZE]${reset}`);
const modePrefix = modes.length > 0 ? modes.join(" ") + " | " : "";

// Build output
let output = modePrefix;
