...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!dependencyReasonFound) {
      ...
      console.error(
        `[Parallelism gate] ⚠️  Missed parallelization opportunity.\n` +
          `  User prompt contains multi-task keywords (${matchedKeywords.join(", ")}) but only 1 Agent call detected.\n` +
          `\n` +
          `  Suggestion: run in ONE single message:\n` +
          `    Agent(subagent_type="${agentType}", description="<task A>", prompt="...")\n` +
          `    Agent(subagent_type="${agentType}", description="<task B>", prompt="...")\n` +
          `\n` +
          `  If there is a real output->input dependency, declare it inline (e.g. "solo delegation — waiting on scout output") and this warning is skipped.`,
      );
